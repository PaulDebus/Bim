...
// Einbindung aller benÃ¶tigten Klassen
	...
	private void updateView() {
		...
		try {
			if (ifcModel.getCollection(IfcWindow.class) != null) {
					...
					//Schleife Ã¼ber das Set siteList, Initialisierung der einzelnen Elemente als Variable siteIt
					for (IfcRelDecomposes siteIt : siteList)
					{
						// Casten von IfcRelDecomposes zu IfcSite, Auslesen der GelÃ¤nde innerhalb des Projektes, 
						// Speichern in Variable site
						IfcSite site = (IfcSite) siteIt.getRelatedObjects().iterator().next();
						Meldung += site.getName();
						
						// Speichern aller Unterwerte des aktuellen GelÃ¤ndes in ein Set
						SET<IfcRelDecomposes> buildingList = site.getIsDecomposedBy_Inverse();
						//Schleife Ã¼ber das Set buildingList, Initialisierung der einzelnen Elemente als Variable buildingIt
						for (IfcRelDecomposes buildingIt : buildingList)
						{
							// Casten, Auslesen der GebÃ¤ude auf dem GelÃ¤nde, Speichern in Variable building
							IfcBuilding building = (IfcBuilding) buildingIt.getRelatedObjects().iterator().next();
							Meldung += "\n GebÃ¤ude: " + building.getName();
							
							// Speichern aller Unterwerte des aktuellen GebÃ¤udes in ein Set
							// get related objects?
							...
							//Schleife Ã¼ber das Set storeyList, Initialisierung der einzelnen Elemente als Variable storeyDefinition
							...
							for (IfcObjectDefinition storey : listedStoreys)
							{
								...
								{
									// Unterscheidung der einzelnen Bauteile Ã¼ber den Namen ihrer Klasse mittels eines switch
									...
									else if (containedInStorey instanceof IfcWindow)
									{
										...
										//Meldung += ", HÃ¶he: " + height.toString() + "m Breite: " + width.value + "m";
									}
